...
import sample.Server.connection.MouseInfo;

import java.awt.*;
import java.awt.event.InputEvent;
import static sample.Server.connection.Server.*;
...
public class GetInput {
    ...
    private void hitKey(KeyEvent keyEvent) {
        if (keyEvent.getEventType().equals(KeyEvent.KEY_PRESSED)) {
            robot.keyPress(keyEvent.getCode().getCode());
        } else if (keyEvent.getEventType().equals(KeyEvent.KEY_RELEASED)) {
            robot.keyRelease(keyEvent.getCode().getCode());
        }
    }
}
